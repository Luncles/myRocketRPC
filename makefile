...
ALL_TESTS : $(PATH_BIN)/test_log $(PATH_BIN)/test_eventloop $(PATH_BIN)/test_tcp $(PATH_BIN)/test_client $(PATH_BIN)/test_rpc_server $(PATH_BIN)/test_rpc_client
...
install:
	mkdir -p $(PATH_INSTALL_INC_COMM) $(PATH_INSTALL_INC_NET) \
		...
		&& cp $(LIB_OUT) $(PATH_INSTALL_INC_ROOT) /
